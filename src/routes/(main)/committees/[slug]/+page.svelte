...
<Back href="/committees">
    Back to Committees
</Back>

<section class="text-center flex flex-col gap-2">
    <h1 class="text-4xl font-bold capitalize">{data.metadata.title}</h1>
    <h2 class="text-slate-400 font-bold text-2xl">{data.metadata.position}</h2>
    <p class="italic">{formatDate(data.metadata.start)} - {data.metadata.end ? formatDate(data.metadata.end) : "present"}</p>
</section>
